...
if ( is_ripon() || is_alumni() ) {
	...
	function person_metaboxes() {
	    ...
	    $person_box->add_field( array(
	        'name' => 'Counselor Counties (WI)',
	        'id' => $prefix . 'person_counties',
	        'type' => 'textarea',
	        'options' => array (
	        	'textarea_rows' => 7
	        )
	...
	function shortcode_counselors() {
		...
		$counselors_dropdown = "<div class='counselor-filters'><div class='counselor-state'><label>Select a State/Territory:</label> <select class='counselor-state-select'>";
		$counselors_dropdown .= "<option value='0'>-- All Counselors --</option>";

		// loop through the states/territories and generate the options
		foreach ( $states as $counselor_state_key => $counselor_state_name ) {
			$counselors_dropdown .= "<option value='" . $counselor_state_key . "'>" . $counselor_state_name . "</option>";
		}

		// end the counselors dropdown
		$counselors_dropdown .= "</select></div><div class='counselor-search'><label>Search Counselors by County:</label><input type='text' class='counselor-search-text' placeholder='Search by County'></div></div>";
		...
		foreach ( $counselors as $a_counselor ) {
			...
			$cstates = get_cmb_value( 'person_states', $a_counselor->ID );
			...
			// store some info
			$person_title = get_cmb_value( 'person_title', $a_counselor->ID );
			$person_phone = get_cmb_value( 'person_phone', $a_counselor->ID );

			// start the output of this person's information
			$counselors_dropdown .= "<div class='counselor" . ( in_array( 'WI', $cstates ) ? ' wi' : '' ) . "'" . 
				"data-states='" . ( !empty( $cstates ) ? implode( ',', $cstates ) : '' ) . "' " . 
				"data-counties='" . ( !empty( get_cmb_value( 'person_counties', $a_counselor->ID ) ) ? get_cmb_value( 'person_counties', $a_counselor->ID ) : '' ) . "'>" .
					"<div class='counselor-photo'><a href='/bio/" . $a_counselor->post_name . "'>" . get_the_post_thumbnail( $a_counselor->ID ) . "</a></div>" . 
					"<div class='counselor-info'>
						<h3><a href='/bio/" . $a_counselor->post_name . "'>" . $a_counselor->post_title . "</a></h3>
						<p>" . 
						( !empty( $person_title ) ? $person_title . "<br>" : "" ) . 
						$person_phone . "</p>
						<p><a href='mailto:" . get_cmb_value( 'person_email', $a_counselor->ID ) . "'>" . get_cmb_value( 'person_email', $a_counselor->ID ) . "</a></p>
						<p class='states'><strong>States/Regions Served:</strong> " . ( !empty( $state_names ) ? implode( ', ', $state_names ) : '' ) . "</p>" . 
						( !empty( get_cmb_value( 'person_counties', $a_counselor->ID ) ) ? "<p class='counties'><strong>Counties (WI) Served:</strong> " . get_cmb_value( 'person_counties', $a_counselor->ID ) . "</p>" : '' ) . 
					"</div>
				</div>";
		}
